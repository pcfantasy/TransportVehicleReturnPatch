...
{
	[HarmonyPatch]
	public static class PassengerFerryAILoadPassengersPatch
	{
		public static MethodBase TargetMethod()
		{
			return typeof(PassengerFerryAI).GetMethod("LoadPassengers", BindingFlags.NonPublic | BindingFlags.Instance);
		}
		public static bool Prefix(ref PassengerFerryAI __instance, ushort vehicleID, ref Vehicle data)
		{
				...
				else if (TransportVehicleReturnPatch.rejectPassengerVehicleID[data.m_transportLine] != 0)
				{
					...
					if (tmpData.m_transportLine != data.m_transportLine)
					{
						if (TransportVehicleReturnPatch.debugMode)
							DebugLog.LogToFileOnly($"Error in PassengerFerryAILoadPassengersPatch: tmpData.m_transportLine = {tmpData.m_transportLine} data.m_transportLine = {data.m_transportLine}");
						TransportVehicleReturnPatch.rejectPassengerVehicleID[data.m_transportLine] = 0;
					}
				}
			}
