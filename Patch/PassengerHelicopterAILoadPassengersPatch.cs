...
{
	[HarmonyPatch]
	public static class PassengerHelicopterAILoadPassengersPatch
	{
		public static MethodBase TargetMethod()
		{
			return typeof(PassengerHelicopterAI).GetMethod("LoadPassengers", BindingFlags.NonPublic | BindingFlags.Instance);
		}
		public static bool Prefix(ref PassengerHelicopterAI __instance, ushort vehicleID, ref Vehicle data)
		{
				...
				else if (TransportVehicleReturnPatch.rejectPassengerVehicleID[data.m_transportLine] != 0)
				{
					...
					if (tmpData.m_transportLine != data.m_transportLine)
					{
						if (TransportVehicleReturnPatch.debugMode)
							DebugLog.LogToFileOnly($"Error in PassengerHelicopterAILoadPassengersPatch: tmpData.m_transportLine = {tmpData.m_transportLine} data.m_transportLine = {data.m_transportLine}");
						TransportVehicleReturnPatch.rejectPassengerVehicleID[data.m_transportLine] = 0;
					}
				}
			}
